...
# -*- coding: utf-8 -*-
"""
CARONTE UPDATE — Atualiza metadata + thumbnail de video ja existente no YouTube
...
def set_thumbnail(youtube, video_id):
    """Define thumbnail do video (comprime antes de upload)"""
    try:
...
"""
    ...
    with open(LOG_FILE, 'a', encoding='utf-8') as lf:
        lf.write(f'[{now}] CARONTE UPDATE -- Atualizacao de metadata para {VIDEO_ID}\n')
        lf.write(f'[{now}] THUMBNAIL: {"OK" if thumb_ok else "ERRO"} | CHAPTERS: {"OK" if desc_ok else "ERRO"}\n')

    print(f'[OK] Status atualizado em {STATUS_FILE}')
...
def main():
    print('='*60)
    print('CARONTE UPDATE — Atualizar Metadata')
    print('='*60)
    ...
    print(f'[OK] Thumbnail encontrada ({thumb_size_mb:.1f} MB) [sera comprimida]')
    ...
    # Update status
    update_status(thumb_ok, desc_ok)

    print('='*60)
    print(f'[SUCESSO] Video {VIDEO_ID} atualizado')
    if thumb_ok and desc_ok:
        print('[OK] Thumbnail e metadata atualizadas via API')
    else:
        if not thumb_ok:
            print('[AVISO] Thumbnail nao foi atualizada')
        if not desc_ok:
            print('[AVISO] Descricao/chapters nao foram atualizadas')
    print('='*60)
