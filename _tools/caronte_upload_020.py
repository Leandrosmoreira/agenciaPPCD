...
video-020-1984-daniel | Sinais do Fim
"""
...
VIDEO_FILE     = r'C:\Users\Leandro\Downloads\agencia\canais\sinais-do-fim\videos\video-020-1984-daniel\7-edicao\0421.mp4'
THUMB_FILE     = r'C:\Users\Leandro\Downloads\agencia\canais\sinais-do-fim\videos\video-020-1984-daniel\8-publicacao\thumb.png'
METADATA_FILE  = r'C:\Users\Leandro\Downloads\agencia\canais\sinais-do-fim\videos\video-020-1984-daniel\8-publicacao\metadata.txt'
STATUS_FILE    = r'C:\Users\Leandro\Downloads\agencia\canais\sinais-do-fim\videos\video-020-1984-daniel\8-publicacao\status_upload.txt'
...
def upload_video(youtube, metadata):
    print('[CARONTE] Iniciando upload (PRIVATE)...')
    body = {
        'snippet': {
        ...
        'status': {
            'privacyStatus': 'private',  # NUNCA public automaticamente
    ...
    try:
        ...
        while quality >= 20:
            ...
            if os.path.getsize(temp_file) < max_size_bytes:
                print(f'[OK] Thumbnail comprimida (qualidade {quality}%)')
                return temp_file
            quality -= 5
        os.unlink(temp_file)
        return None
...
STATUS: private
UPLOAD_TIME: {now}
THUMBNAIL: {'OK' if thumb_ok else 'ERRO — definir manualmente via caronte_post_upload.py'}
...
"""
    ...
    with open(LOG_FILE, 'a', encoding='utf-8') as lf:
        lf.write(f'[{now}] CARONTE — video-020-1984-daniel upload OK (PRIVATE) -> {video_id}\n')
        lf.write(f'[{now}] CHECKPOINT 4 CONCLUIDO — aguardando aprovacao Snayder para public\n')
...
def main():
    print('='*60)
    print('CARONTE — video-020-1984-daniel')
    print('='*60)
    ...
    if not os.path.exists(METADATA_FILE):
        print(f'[ERRO] Metadata nao encontrado')
        return False
    print('[OK] Metadata encontrada')

    has_thumb = os.path.exists(THUMB_FILE)
    if not has_thumb:
        print(f'[AVISO] Thumbnail nao encontrada em {THUMB_FILE}')
        print('[AVISO] Upload prosseguira sem thumbnail — definir depois via YouTube Studio')
    ...
    if not metadata['titulo']:
        print('[ERRO] Titulo nao encontrado em metadata.txt')
        return False
    ...
    thumb_ok = set_thumbnail(youtube, video_id) if has_thumb else False
    ...
    print('STATUS: PRIVATE — aguardando aprovacao de Snayder')
    print('='*60)
    return True
